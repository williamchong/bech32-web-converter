...
Nuxt 4 single-page app (`app/pages/index.vue`) deployed on GitHub Pages (`.github/workflows/deploy.yml`). Source code lives under the Nuxt 4 `app/` directory (`app/components/`, `app/layouts/`, `app/pages/`, `app/plugins/`, `app/utils/`, `app/app.config.ts`); `server/`, `i18n/`, `public/`, and `nuxt.config.ts` stay at the project root. Requires Node.js >= 24.11.0.

### Core conversion logic

`app/utils/address.ts` — two main functions:
...
These depend on `bech32` and `keccak256` npm packages, which require Node.js `Buffer`. A client-side polyfill in `app/plugins/node.client.ts` makes `Buffer` globally available in the browser. The `experimental.clientNodeCompat` flag and custom aliases in `nuxt.config.ts` handle additional Node module resolution for the browser environment.
